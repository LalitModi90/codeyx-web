    ...
    async function fetchProblems() {
      if (selectedSheet) {
        try {
          ...
          console.error("Failed to load from Supabase, falling back to mock data", err);
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 bg-white/[0.02] border border-white/5 p-2 rounded-2xl">
                <div className="flex items-center gap-2 w-full md:w-auto">
                  ...
                  <button className="p-2 rounded-xl bg-[#080710] border border-white/10 text-gray-400 hover:text-white transition-colors md:hidden">
                    <Filter size={16} />
                  </button>
                </div>
                                      ...
                                      <tbody>
                                        {diffs[diff].map((prob:any) => (
                                          <tr key={prob.id} className="border-b border-white/[0.02] last:border-none hover:bg-white/[0.02] transition-colors group">
                                            ...
                                            <td className="py-3 px-4 text-center">
                                              <button className="text-gray-600 hover:text-orange-400 transition-colors inline-flex justify-center"><FileText size={14} /></button>
                                            </td>
                                            <td className="py-3 px-4 text-center">
                                              <button className="text-gray-600 hover:text-rose-500 transition-colors inline-flex justify-center"><Video size={14} /></button>
                                            </td>
                                            <td className="py-3 px-4 text-center">
                                              <button className="text-gray-600 hover:text-emerald-400 transition-colors inline-flex justify-center"><ExternalLink size={14} /></button>
                                            </td>
                                            <td className="py-3 px-4 text-center">
                                              <button className="text-gray-600 hover:text-white transition-colors inline-flex justify-center"><BookOpen size={14} /></button>
                                            </td>
                                            <td className="py-3 px-4 text-center">
                                              <button className={`inline-flex justify-center transition-colors ${prob.status === 'Revision Pending' ? 'text-yellow-400' : 'text-gray-600 hover:text-yellow-400'}`}>
