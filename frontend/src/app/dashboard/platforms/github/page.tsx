...
const repositoriesList = [
  { name: 'Codeyx', desc: 'A platform to track coding progress and analyze performance across platforms.', stars: 128, forks: 24, updated: '2d ago', tech: 'TypeScript', isPublic: true, demoUrl: 'https://codesync.dev', commits: 450 },
...
const languagesMap: Record<string, { pct: string, numPct: number, projects: number, commits: number, color: string, strokeColor: string, desc: string, repos: string[] }> = {
  'TypeScript': { pct: '35%', numPct: 35, projects: 12, commits: 1420, color: '#a855f7', strokeColor: '#a855f7', desc: 'Used extensively in enterprise dashboards, custom APIs, and backend telemetry wrappers.', repos: ['Codeyx', 'DevAI Chat', 'Quick-Scraper'] },
...
export default function GitHubDashboard() {
  ...
  return (
    <div className={"min-h-screen " + bg + " " + txt + " transition-colors duration-300 font-sans"} >
      <TopNavbar />

      <div className="flex h-[calc(100vh-64px)]">

        {/* LEFT DEVELOPER SIDEBAR */}
        <aside className={`hidden lg:flex flex-col w-56 border-r ${border} ${sidebar} flex-shrink-0 overflow-y-auto scrollbar-none`}>
          {/* DEVELOPER Section */}
          <div className="px-4 pt-6 pb-2">
            <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest block mb-3 pl-1">Developer</span>
            <div className="space-y-0.5">
              {[
                { icon: FolderGit2, label: 'Overview', tab: 'Overview' },
                { icon: BookOpen, label: 'Repositories', tab: 'Repositories' },
                { icon: Activity, label: 'Contributions', tab: 'Contributions' },
                { icon: Code, label: 'Languages', tab: 'Languages' },
                { icon: Flame, label: 'Activity', tab: 'Activity' },
                { icon: Globe2, label: 'Open Source', tab: 'Open Source' },
                { icon: TrendingUp, label: 'Performance', tab: 'Performance' },
              ].map((item) => (
                <button
                  key={item.tab}
                  onClick={() => handleTabChange(item.tab)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 group ${activeTab === item.tab
                      ? 'bg-[#FF8A00]/10 text-[#FF8A00] border border-[#FF8A00]/20'
                      : 'text-gray-400 hover:text-white hover:bg-white/[0.04] border border-transparent'
                    }`}
                >
                  <item.icon size={14} className={activeTab === item.tab ? 'text-[#FF8A00]' : 'text-gray-500 group-hover:text-[#FF8A00] transition-colors'} />
          ...
          {/* GITHUB Section */}
          <div className={`mx-4 mt-4 p-3 rounded-xl border ${border} bg-white/[0.02]`}>
            <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest block mb-3">GitHub</span>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              <span className="text-xs font-bold text-white">Connected to GitHub</span>
            </div>
            <div className="text-[9px] text-gray-500 font-semibold mb-3">@ankitkumar01</div>
            <button className="w-full py-2 rounded-lg bg-[#FF8A00]/10 border border-[#FF8A00]/20 text-[#FF8A00] text-[10px] font-bold hover:bg-[#FF8A00]/20 transition-all flex items-center justify-center gap-1.5">
              <RefreshCw size={10} /> Sync Again
            </button>
            <div className="text-[8px] text-gray-600 font-semibold mt-2 text-center">Last synced 2 hours ago</div>
          </div>

          {/* CONNECTED PLATFORMS Section */}
          <div className="px-4 mt-4">
            <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest block mb-3 pl-1">Platforms</span>
            <div className="space-y-1">
              {connectedPlatforms.map((plat) => (
                ...
                >
                  <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${plat.active ? 'bg-[#FF8A00]' : 'bg-emerald-400'}`} />
                  <span className={`text-xs font-semibold ${plat.active ? 'text-[#FF8A00]' : 'text-gray-400 group-hover:text-white transition-colors'}`}>{plat.name}</span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <main className="flex-1 p-6 overflow-y-auto space-y-6 scrollbar-none pb-16">
            ...
            <div className="flex items-center mb-1 animate-fade-in">
              <Link
                ...
                className="inline-flex items-center gap-1.5 text-[10px] font-black text-gray-500 hover:text-[#FF8A00] uppercase tracking-widest transition-colors group cursor-pointer"
              >
                <ArrowRight size={12} className="rotate-180 text-gray-500 group-hover:text-[#FF8A00] transition-transform duration-200 group-hover:-translate-x-0.5" />
            ...
            <div className="border-b border-white/5 flex items-center gap-6">
              {['Overview', 'Repositories', 'Contributions', 'Languages', 'Activity', 'Open Source', 'Performance'].map((tab) => (
                <button
                  ...
                  className={`pb-3.5 text-xs font-bold transition-all relative ${activeTab === tab ? 'text-[#FF8A00] font-extrabold' : 'text-gray-400 hover:text-white'
                    }`}
                >
                  <span>{tab}</span>
                  {activeTab === tab && (
                    <motion.div
                      ...
                      className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#FF8A00] shadow-[0_0_8px_rgba(255,138,0,0.8)]"
                    />
                  )}
                </button>
              ))}
                  ...
                  >
                    ...
                    {activeTab === 'Overview' && (
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                          {[
                            { title: 'Total Repositories', value: '42', sub: '▲ 4 this month', color: 'text-[#FF8A00]', icon: FolderGit2 },
                        ...
                        <div className={`${card} rounded-2xl p-5 border`}>
                          <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/5">
                            <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider">Contribution Heatmap</span>
                                      ...
                                      <div key={wIdx} className="flex flex-col gap-[3.5px]">
                                        {week.map((cell, cIdx) => {
                                          ...
                                          return (
                                            ...
                                            >
                                              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 hidden group-hover:block z-50 bg-[#0b0a12] border border-[#FF8A00]/20 px-2 py-1 rounded-lg text-[8px] font-mono text-orange-300 shadow-xl whitespace-nowrap">
                        ...
                        <div className="space-y-3">
                          <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider block">Top Showcase Repositories</span>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {repositoriesList.slice(0, 3).map(repo => (
                              <div key={repo.name} className={`${card} rounded-2xl p-4 border flex flex-col justify-between hover:border-[#FF8A00]/30 transition-all`}>
                                <div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center bg-[#0b0a12]/60 border border-white/5 p-4 rounded-2xl">
                          <div className="flex-1 w-full relative">
                            ...
                            <input
                              ...
                              className="w-full bg-[#0f141d] border border-white/5 rounded-xl pl-10 pr-4 py-2 text-xs text-white focus:outline-none focus:border-[#FF8A00]/40 font-semibold transition-all"
                            />
                          </div>
                          ...
                          <div className="flex items-center gap-3 w-full md:w-auto">
                            <div className="flex items-center gap-1.5 bg-[#0f141d] border border-white/5 rounded-xl px-3 py-2 text-xs font-bold text-gray-400">
                              <Filter size={12} className="text-[#FF8A00]" />
                              <select
                                value={repoLanguage}
                            ...
                            <div className="flex items-center gap-1.5 bg-[#0f141d] border border-white/5 rounded-xl px-3 py-2 text-xs font-bold text-gray-400">
                              <ArrowUpDown size={12} className="text-[#FF8A00]" />
                              <select
                                value={repoSort}
                        ...
                        {filteredRepos.length > 0 ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            {filteredRepos.map(repo => (
                              <div key={repo.name} className={`${card} rounded-2xl p-4 border flex flex-col justify-between hover:border-[#FF8A00]/30 transition-all group`}>
                                <div>
                                  <div className="flex justify-between items-center gap-2">
                                    <span className="text-xs font-extrabold text-white truncate group-hover:text-[#FF8A00] transition-colors">{repo.name}</span>
                                    <span className={`text-[7.5px] font-black px-1.5 py-0.5 rounded-full border leading-none ${repo.isPublic
                                ...
                                <div className="mt-4 pt-3.5 border-t border-white/5 space-y-3">
                                  <div className="flex justify-between items-center text-[8.5px] font-mono font-bold text-gray-500">
                                    ...
                                    <span className="flex items-center gap-1"><GitFork size={10} className="text-[#FF8A00]" />{repo.forks} forks</span>
                                  </div>
                                  ...
                                  <div className="flex items-center justify-between text-[7.5px] font-extrabold text-gray-400 bg-white/[0.01] border border-white/5 p-2 rounded-lg">
                                    ...
                                    <span>Commits: <span className="text-[#FF8A00]">{repo.commits}</span></span>
                                  </div>

                                  <div className="flex items-center gap-2 pt-1">
                                    <button className="flex-1 py-1.5 bg-[#161b22] hover:bg-[#21262d] border border-white/5 hover:border-[#FF8A00]/20 text-white rounded-lg text-[9px] font-bold flex items-center justify-center gap-1 transition-all">
                                      <span>Repository</span>
                                      <ExternalLink size={8} />
                                    </button>
                                    {repo.demoUrl && (
                                      <Link href={repo.demoUrl} target="_blank" className="p-1.5 bg-[#FF8A00]/10 border border-[#FF8A00]/20 hover:border-[#FF8A00]/40 rounded-lg text-[#FF8A00] hover:text-white transition-all flex items-center justify-center">
                                        <Eye size={12} />
                                      </Link>
                                    )}
                                  </div>
                    ...
                    {activeTab === 'Contributions' && (() => {
                      ...
                      return (
                        <div className="space-y-6">
                          ...
                          <div className={`${card} rounded-2xl p-5 border`}>
                            <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/5">
                              <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider">GitHub heatmap (gregorian perfect)</span>
                            ...
                            <div className="mt-4 pt-3.5 border-t border-white/5 flex items-center justify-between min-h-[30px] bg-[#0b0a12]/40 rounded-xl px-4 py-2 border border-white/[0.02]">
                              {hoveredHeatmapDay ? (
                                <div className="flex items-center gap-2 text-[9.5px] font-bold text-gray-200">
                                  ...
                                  <span>
                                    Selected: <span className="text-[#FF8A00] font-extrabold">{hoveredHeatmapDay.count} commits</span> logged on <span className="text-white font-extrabold">{new Date(hoveredHeatmapDay.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                                  </span>
                                </div>
                              ) : (
                                <span className="text-[9px] text-gray-500 font-extrabold flex items-center gap-1">
                                  <Sparkles size={11} className="text-[#FF8A00]/80 animate-pulse" />
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            ...
                            <div className={`${card} rounded-2xl p-5 border flex flex-col justify-between relative overflow-hidden`}>
                              <div className="mb-4 flex items-center justify-between">
                                <div>
                                  <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider block">Commits per Month</span>
                                ...
                                {monthlyCommitsDataset.map(m => (
                                  ...
                                  >
                                    ...
                                    {hoveredMonthBar === m.label && (
                                      <div className="absolute bottom-full mb-2 bg-[#0b0a12]/95 border border-[#FF8A00]/30 p-2 rounded-lg shadow-2xl z-50 pointer-events-none text-center min-w-[120px] transition-all">
                                        <span className="text-[9px] font-black text-white block">{m.label} Commits</span>
                                        <span className="text-[10px] font-black text-[#FF8A00] block font-mono mt-0.5">{m.value} logged</span>
                                      </div>
                                    )}

                                    <div className={`w-full rounded-xl flex items-end h-32 overflow-hidden border transition-all duration-200 ${hoveredMonthBar === m.label
                              ...
                              <div className="mt-4 pt-3 border-t border-white/5 min-h-[36px]">
                                {hoveredMonthBar ? (
                                  <p className="text-[9.5px] text-gray-400 font-semibold leading-relaxed">
                                    <span className="text-[#FF8A00] font-black uppercase tracking-wider mr-1.5">Info:</span>
                            ...
                            <div className={`${card} rounded-2xl p-5 border flex flex-col justify-between relative`}>
                              <div className="mb-4 flex items-center justify-between">
                                <div>
                                  <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider block">Active Coding Hours</span>
                              ...
                              <div className="h-40 flex items-end justify-between gap-1.5 pt-6 select-none relative">
                                {activeHoursDataset.map(h => (
                                  ...
                                  >
                                    ...
                                    <div className={`w-full rounded-lg flex items-end h-32 overflow-hidden border transition-all duration-200 ${hoveredHourBar === h.label
                    ...
                    {activeTab === 'Languages' && (
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                          ...
                          <div className={`${card} lg:col-span-5 rounded-2xl p-5 border flex flex-col justify-between`}>
                            <div className="mb-4 pb-2 border-b border-white/5">
                              <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider">Interactive Language Donut</span>
                            </div>
                            ...
                            <div className="flex items-center justify-center py-6">
                              <div className="relative w-40 h-40">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center select-none text-center p-2">
                                  ...
                                  <span className="text-base font-extrabold text-[#FF8A00] font-mono mt-0.5 leading-none">{currentLang.pct}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className={`${card} lg:col-span-7 rounded-2xl p-5 border flex flex-col justify-between`}>
                            <div className="mb-4 pb-2 border-b border-white/5 flex items-center justify-between">
                              <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider">Usage & Project Metrics</span>
                            ...
                            <div className="space-y-2">
                              {Object.entries(languagesMap).map(([name, l]) => (
                                <div
                                  ...
                                  className={`flex items-center justify-between p-2.5 rounded-xl border transition-all duration-200 cursor-pointer ${selectedLanguageHover === name
                                      ? 'border-[#FF8A00]/40 bg-[#FF8A00]/5 shadow-md scale-[1.01]'
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-5 text-[9.5px] font-mono font-bold text-gray-400">
                                    ...
                                    <span>Projects: <span className="text-[#FF8A00]">{l.projects}</span></span>
                        ...
                        <div className={`${card} rounded-2xl p-5 border shadow-2xl relative overflow-hidden group`}>
                          <div className="absolute top-0 right-0 w-32 h-32 bg-[#FF8A00]/[0.02] rounded-full blur-2xl pointer-events-none" />
                          <div className="flex items-center gap-2 mb-3">
                          ...
                          <div className="mt-4 pt-3.5 border-t border-white/5 flex flex-wrap items-center gap-2.5">
                            ...
                            {currentLang.repos.map(r => (
                              <span
                                key={r}
                                className="px-2.5 py-1 bg-[#161b22] hover:bg-[#21262d] border border-white/5 hover:border-[#FF8A00]/20 text-white rounded-lg text-[9px] font-extrabold transition-all cursor-pointer shadow-[0_0_8px_rgba(255,255,255,0.02)]"
                              >
                                {r}
                              </span>
                            ))}
                    ...
                    {activeTab === 'Activity' && (
                      <div className="space-y-5">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider">GitHub Activity Feed</span>
                          <span className="text-[8px] text-gray-500 font-bold">Realtime telemetries scraper</span>
                        </div>

                        <div className="space-y-3">
                          {[
                            { text: 'Pushed 5 commits to Codeyx frontend page scraper', time: '2 hours ago', type: 'commit', repo: 'Codeyx' },
                            { text: 'Opened Pull Request #14 inside devai-chat branch configurations', time: '4 hours ago', type: 'pr_open', repo: 'DevAI-Chat' },
                            { text: 'Merged Pull Request #12: Fix resolve auth rate limits', time: '1 day ago', type: 'pr_merge', repo: 'Codeyx' },
                            ...
                            <div key={idx} className={`${card} rounded-xl p-3.5 border flex items-center justify-between group hover:border-[#FF8A00]/20 transition-all`}>
                              <div className="flex items-center gap-3">
                                <div className="p-2 rounded-xl bg-[#FF8A00]/10 border border-[#FF8A00]/20 text-[#FF8A00]">
                                ...
                                <div>
                                  <span className="text-xs font-bold text-gray-200 group-hover:text-[#FF8A00] transition-colors block">{act.text}</span>
                    ...
                    {activeTab === 'Open Source' && (
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          {[
                            { label: 'OSS Repos Contributed', value: '32', color: 'text-[#FF8A00]', sub: '▲ 4 this month' },
                        ...
                        <div className="space-y-3">
                          <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider block">OSS Achievements & Badges</span>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                              { name: 'Pull Shark', desc: 'Opened 50+ pull requests on public repositories.', badge: '🦈 Pull Shark Badge', color: 'from-orange-500 to-amber-600' },
                    ...
                    {activeTab === 'Performance' && (
                      <div className="space-y-6">
                        ...
                        <div className="flex justify-between items-center bg-[#0b0a12]/60 border border-white/5 p-3 rounded-2xl">
                          <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider pl-1">Performance Timeframe</span>
                          <div className="flex gap-1.5 bg-[#0f141d] p-1 border border-white/5 rounded-xl">
                            {['7D', '30D', '90D'].map(range => (
                              <button
                                key={range}
                                onClick={() => setPerformanceRange(range)}
                                className={`px-3 py-1 rounded-lg text-[10px] font-extrabold uppercase transition-all ${performanceRange === range
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className={`${card} rounded-2xl p-5 border flex flex-col justify-between hover:border-[#FF8A00]/20 transition-all`}>
                          ...
                          <div className={`${card} rounded-2xl p-5 border flex flex-col justify-between hover:border-[#FF8A00]/20 transition-all`}>
                            ...
                            <div className="py-4">
                              ...
                              <span className="block text-[8.5px] text-[#FF8A00] font-bold mt-1.5">{currentPerf.trendSub}</span>
                            </div>
                          </div>

                          {/* AI INSIGHTS CARD */}
                          <div className={`${card} rounded-2xl p-5 border flex flex-col justify-between relative overflow-hidden group hover:border-[#FF8A00]/20 transition-all`}>
                            <div className="absolute -top-12 -right-12 w-24 h-24 bg-[#FF8A00]/10 rounded-full blur-2xl group-hover:scale-125 transition-transform duration-500" />
                            <span className="text-[8.5px] font-black text-[#FF8A00] uppercase tracking-widest leading-none flex items-center gap-1">
                              <Brain size={12} className="text-[#FF8A00] animate-pulse" />
                              <span>AI Telemetry Insight</span>
                            </span>
                            <div className="py-3">
                              <span className="text-xs font-extrabold text-white block">{currentPerf.aiSub}</span>
                              <span className="text-lg font-black text-orange-300 block mt-1">{currentPerf.aiInsight}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className={`${card} rounded-2xl p-5 border relative overflow-hidden`}>
                          <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/5">
                            <div>
                              <span className="text-xs font-black text-[#FF8A00] uppercase tracking-wider block">Repository Growth Track</span>
                          ...
                          <div className="h-44 relative pt-4 select-none">
                            ...
                            {activePoint && (
                              <div
                                className="absolute bg-[#0b0a12]/95 border border-[#FF8A00]/30 p-2.5 rounded-xl shadow-2xl z-50 pointer-events-none transition-all duration-100 flex flex-col gap-1 text-[9.5px]"
                                style={{
                              ...
                              >
                                ...
                                <div className="flex flex-col gap-0.5 text-gray-400 font-semibold font-mono">
                                  ...
                                  <span>Commits Made: <span className="text-[#FF8A00] font-extrabold">{activePoint.commits}</span></span>
